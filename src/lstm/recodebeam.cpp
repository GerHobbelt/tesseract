...
std::string RecodeNode::Print(int null_char, const UNICHARSET *unicharset, int depth) const {
  std::string s;
  if (code == null_char) {
    return "null_char";
  } else {
...
void RecodeBeamSearch::DecodeSecondaryBeams(
  ...
  for (int t = 0; t < width; ++t) {
    ...
    ComputeSecTopN(&(excludedUnichars)[bucketNumber], output.f(t), output.NumFeatures(), kBeamWidths[0]);
...
void RecodeBeamSearch::ExtractBestPathAsWords(const TBOX &line_box,
  ...
  // it.
  ExtractPathAsUnicharIds(best_nodes, &unichar_ids, &certs, &ratings, &xcoords, &character_boundaries_, &unichar_codes);    // JDWORIG
...
void RecodeBeamSearch::DebugBeamPos(const UNICHARSET *unicharset,
  ...
  std::vector<const RecodeNode *> unichar_bests(unicharset ? unicharset->size() : 256 /* ASCII range + high ASCII ? */);
  ...
  if (t == 0) {
    ...
    if (HasDebug()) {
      ...
      for (int i = prev->beams_[beam_index].size() - 1; i >= 0; --i) {
        std::vector<const RecodeNode *> path;
        ExtractPath(&prev->beams_[beam_index].get(i).data(), &path);
        tprintDebug("Step {}: Non-Dawg beam {}:\n", t, i);
        DebugPath(charset, path);
      }
    }
    int total_beam = 0;
...
void RecodeBeamSearch::DebugUnicharPath(
  ...
  for (unsigned c = 0; c < num_ids; ++c) {
    int coord = xcoords[c];
	auto unichar_id = unichar_ids[c];
    tprintDebug("{} {}={} r={}, c={}, s={}, e={}, perm={}\n", coord, unichar_id,
			(unicharset ?
				unicharset->debug_str(unichar_id).c_str() :
				std::string(1, (unichar_id < 0x80 && isprint(unichar_id)) ? (char)unichar_id : '.').c_str()),
            ratings[c], certs[c],
