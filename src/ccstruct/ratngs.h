...
public:
  ...
  inline UNICHAR_ID unichar_id(unsigned int index) const {
    assert(index < length_);
    return unichar_ids_[index];
  }
  inline unsigned int state(unsigned int index) const {
    return state_[index];
  }
  ScriptPos BlobPosition(unsigned int index) const {
    if (index >= length_) {
      return SP_NORMAL;
    }
  ...
  inline float certainty(unsigned int index) const {
    return certainties_[index];
  }
  inline void set_certainty(unsigned int index, float new_val) {
	  certainties_[index] = new_val;
  }
  ...
  BLOB_CHOICE_LIST *blob_choices(unsigned int index, MATRIX *ratings) const;

  // Returns the MATRIX_COORD corresponding to the location in the ratings
  // MATRIX for the given index into the word.
  MATRIX_COORD MatrixCoord(unsigned int index) const;

  inline void set_unichar_id(UNICHAR_ID unichar_id, unsigned int index) {
    assert(index < length_);
  ...
  inline void init(unsigned int reserved) {
    reserved_ = reserved;
    if (reserved > 0) {
  ...
  void set_blob_choice(unsigned int index, int blob_count, const BLOB_CHOICE *blob_choice);

  bool contains_unichar_id(UNICHAR_ID unichar_id) const;
  void remove_unichar_ids(unsigned int index, int num);
  inline void remove_last_unichar_id() {
    --length_;
  }
  inline void remove_unichar_id(unsigned int index) {
  ...
  void punct_stripped(unsigned int *start_core, unsigned int *end_core) const;
  ...
  unsigned int TotalOfStates() const;
...
private:
  ...
  unsigned int reserved_;            // size of the above arrays
  unsigned int length_;              // word length
