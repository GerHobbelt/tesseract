...
void AssociateUtils::ComputeStats(int col, int row, const AssociateStats *parent_stats,
  ...
  if (debug) {
    tprintf("AssociateUtils::ComputeStats() for col={}, row={}{}\n", col, row,
  ...
  if (fixed_pitch && blob_row != nullptr) {
    ...
    if (debug) {
      tprintf("normalizing height = {} (scale {} xheight {} ascenders {})\n", normalizing_height,
              word_res->denorm.y_scale(), blob_row->x_height(), blob_row->ascenders());
    }
  }
  ...
  if (debug) {
    tprintf("wh_ratio={} (max_char_wh_ratio={}) gap_sum={} {}\n", wh_ratio, max_char_wh_ratio,
  ...
  if (fixed_pitch) {
    ...
    if (debug) {
      tprintf("full_wh_ratio {} full_wh_ratio_total {} full_wh_ratio_var {}\n",
