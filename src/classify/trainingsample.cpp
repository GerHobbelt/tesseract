...
void TrainingSample::ExtractCharDesc(int int_feature_type, int micro_type, int cn_type,
  ...
  if (char_features == nullptr) {
    tprintError("No features to train on of type {}\n", kIntFeatureType);
    num_features_ = 0;
    features_ = nullptr;
  } else {
  ...
  if (char_features == nullptr) {
    tprintError("No CN feature to train on.\n");
  } else {
  ...
  if (char_features == nullptr) {
    tprintError("No Geo feature to train on.\n");
  } else {
    ASSERT_HOST(char_features->NumFeatures == 1);
...
void TrainingSample::DisplayFeatures(DebugView::Color color, ScrollViewReference &window) const {
  for (uint32_t f = 0; f < num_features_; ++f) {
