...
bool NetworkBuilder::InitNetwork(int num_outputs, const char *network_spec, int append_index,
  ...
  if (append_index >= 0) {
    ...
    if (bottom_series == nullptr || top_series == nullptr) {
      tprintError("Yikes! Splitting current network failed!!\n");
      return false;
    }
...
Network *NetworkBuilder::ParseInput(const char **str) {
  ...
  if (num_converted != 4 && num_converted != 5) {
    tprintError("Must specify an input layer as the first layer, not {}!!\n", *str);
    return nullptr;
  }
  *str += length;
...
Network *NetworkBuilder::ParseSeries(const StaticShape &input_shape, Input *input_layer,
  ...
  if (**str != ']') {
    tprintError("Missing ] at end of [Series]!\n");
    delete series;
    return nullptr;
  }
  ++*str;
...
Network *NetworkBuilder::ParseParallel(const StaticShape &input_shape, const char **str) {
  ...
  if (**str != ')') {
    tprintError("Missing ) at end of (Parallel)!\n");
    delete parallel;
    return nullptr;
  }
  ++*str;
...
Network *NetworkBuilder::ParseR(const StaticShape &input_shape, const char **str) {
  ...
  *str = end;
  ...
  for (int i = 0; i < replicas; ++i) {
    ...
    if (network == nullptr) {
      tprintError("Invalid replicated network!\n");
      delete parallel;
      return nullptr;
    }
    parallel->AddToStack(network);
...
Network *NetworkBuilder::ParseC(const StaticShape &input_shape, const char **str) {
  ...
  if (type == NT_NONE) {
    tprintError("Invalid nonlinearity on C-spec!: {}\n", *str);
    return nullptr;
  }
  int y = 0, x = 0, d = 0;
  char *end;
...
Network *NetworkBuilder::ParseLSTM(const StaticShape &input_shape, const char **str) {
  ...
  if (key == 'S') {
    ...
    dim = (*str)[2];
    if (dim != 'x' && dim != 'y') {
      tprintError("Invalid dimension (x|y) in L Spec!:{}\n", *str);
      return nullptr;
    }
    chars_consumed = 3;
    ...
    tprintError("Invalid direction (f|r|b) in L Spec!:{}\n", *str);
    return nullptr;
  }
  char *end;
  int num_states = strtol(*str + chars_consumed, &end, 10);
  if (num_states <= 0) {
    tprintError("Invalid number of states in L Spec!:{}\n", *str);
    return nullptr;
  }
  *str = end;
...
static Network *BuildFullyConnected(const StaticShape &input_shape, NetworkType type,
  ...
  if (input_shape.height() == 0 || input_shape.width() == 0) {
    tprintError("Fully connected requires positive height and width, had {},{}\n", input_shape.height(),
            input_shape.width());
    return nullptr;
  }
...
Network *NetworkBuilder::ParseFullyConnected(const StaticShape &input_shape, const char **str) {
  ...
  NetworkType type = NonLinearity((*str)[1]);
  if (type == NT_NONE) {
    tprintError("Invalid nonlinearity on F-spec!: {}\n", *str);
    return nullptr;
  }
  char *end;
  int depth = strtol(*str + 2, &end, 10);
  if (depth <= 0) {
    tprintError("Invalid F spec!:{}\n", *str);
    return nullptr;
  }
  *str = end;
  std::string name(spec_start, *str - spec_start);
...
Network *NetworkBuilder::ParseOutput(const StaticShape &input_shape, const char **str) {
  ...
  if (dims_ch != '0' && dims_ch != '1' && dims_ch != '2') {
    tprintError("Invalid dims (2|1|0) in output spec!:{}\n", *str);
    return nullptr;
  }
  char type_ch = (*str)[2];
  if (type_ch != 'l' && type_ch != 's' && type_ch != 'c') {
    tprintError("Invalid output type (l|s|c) in output spec!:{}\n", *str);
    return nullptr;
  }
  char *end;
  int depth = strtol(*str + 3, &end, 10);
  if (depth != num_softmax_outputs_) {
    tprintWarn("Given outputs {} not equal to unicharset of {}\n", depth,
  ...
  *str = end;
  ...
  if (input_shape.height() == 0) {
    tprintError("Fully connected requires fixed height!\n");
    return nullptr;
  }
